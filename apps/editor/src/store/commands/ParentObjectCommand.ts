...
> extends AbstractCommand {
    ...
    perform(store: SceneModel, _2: SetStoreFunction<SceneModel>): void {
        ...
        if (this.newParentId !== this.oldParentId) {
            ...
            if (assertDefined(this, setParent, 'setParent')) {
                setParent(
                    produce((parent) =>
                        arrayRemoveEl(parent.children, object.id),
                    ),
                );
            }
            parentobject = store.objects.get(this.newParentId);
        }
        if (!assertDefined(this, parentobject, 'parentobject (for new parent)'))
            return;
        ...
        if (!parentSetter)
            throw new Error(
                `ChangeObjectOrderCommand: Could not get parent setter for ${parentobject.id}.`,
            );
        parentSetter(
            produce((parent) => {
    ...
    undo(store: SceneModel, _2: SetStoreFunction<SceneModel>): void {
        const object = store.objects.get(this.objectId);
        if (!object) {
            throw new Error(
        ...
        if (!parentSetter)
            throw new Error(
                `ChangeObjectOrderCommand: (undo) Could not get parent setter for ${parentobject.id}.`,
            );
        parentSetter(
            produce((parent) => {
