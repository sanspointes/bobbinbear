...
export function Tree<T extends BaseDragDroppable>(props: TreeProps<T>) {
    ...
    return (
        <TreeContext.Provider value={contextData}>
...
function TreeNode<T extends BaseDragDroppable>(props: TreeNodeProps<T>) {
    ...
    // eslint-disable-next-line solid/reactivity
    const droppable = treeCtx.isDroppablePredicate(props.node)
        ? createDroppable(props.node.id, props.node)
        : ((() => {
              return undefined;
          }) as unknown as ReturnType<typeof createDroppable>);

    return (
        <div
            use:draggable={draggable}
            use:droppable={
                treeCtx.currentlyDragging() !== props.node.id
                    ? droppable
                    : undefined
            }
        >
            {treeCtx.nodeTemplate(props.node, () => (
