...
export function CommandStack(props: CommandStackProps) {
    return (
        <Popover
            trigger={
                <Button variant="secondary" inverted={true}>
                    Command Stack ({props.stack.length})
                </Button>
            }
            title="Command Stack"
            class={clsx(
                'overflow-y-scroll pr-2 w-[600px] max-h-[400px]',
                props.class,
            )}
        >
        ...
        <KCollapsible.Root class="w-full">
            ...
            <KCollapsible.Content class="pl-6">
                <Show
                    ...
                    fallback={
                        <pre class="text-xs bg-orange-200">
                            {JSON.stringify(props.command, undefined, 2)}
                        </pre>
                    }
                >
