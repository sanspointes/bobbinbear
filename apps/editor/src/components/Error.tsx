...
const ErrorReason = (props: ErrorReasonProps) => {
  return (
    <KCollapsible.Root class="overflow-hidden mb-2 w-full rounded-md bg-yellow-950">
      <KCollapsible.Trigger class="w-full text-left">
        <p class="text-yellow-200 p-4">
      ...
      <KCollapsible.Content class="pl-6 overflow-scroll">
        <Show when={props.error.stack}>
          {(stack) => <pre class="text-yellow-200 text-xs">{stack()}</pre>}
        </Show>
...
export const ErrorView = (props: ErrorProps) => {
  ...
  return (
    <div class="flex justify-center p-8 w-full h-full bg-yellow-900">
      <div class="relative w-[90%] max-w-[600px] h-full">
        ...
        >
          ...
          <p class="mb-6 w-full text-yellow-200">
        ...
        <Show when={showExtra()}>
          <div class="flex absolute bottom-6 left-1/2 flex-col gap-2 items-start w-full text-sm -translate-x-1/2 bg-yellow-900 max-h-[80vh] py-4">
