...
export type Transitions<STATE, EVENT> = Map<
    EVENT,
    {
        toState: STATE;
        cb: Callback;
...
export class StateMachine<STATE, EVENT> {
    ...
    constructor(_init: STATE, transitions: ITransition<STATE, EVENT>[] = []) {
        this._current = _init;
...
export type ExclusiveStateMachineResult<TState, TEvent> = StateMachineResult<
    TState,
    TEvent
> & {
    ...
    return {
        ...
        dispatch: (event, ...extraArgs) => {
            ...
            if (!needsExclusive() && next && o.exclusiveStates.includes(next)) {
                ...
                needsExclusive() &&
                next &&
                !o.exclusiveStates.includes(next)
            ) {
