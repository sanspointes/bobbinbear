 ...
 */
export type MathType<T extends MathRepresentation | core.Color> = T extends core.Color ? ConstructorParameters<typeof core.Color> | core.Color | core.ColorSource : T extends MathRepresentation ? T | Parameters<T['set']> | number : T;
...
type WithMathProps<P> = {
    [K in keyof P]: P[K] extends MathRepresentation ? MathType<P[K]> : P[K];
};
...
export type ElementProps<T extends ConstructorRepresentation, P = InstanceType<T>> = Partial<Overwrite<WithMathProps<P>, NodeProps<P> & EventProps>>;
export type PixiElement<T extends ConstructorRepresentation> = Mutable<Overwrite<ElementProps<T>, Omit<InstanceProps<InstanceType<T>, T>, 'object'>>>;
...
export type ContainerNode<T> = Overwrite<Node<T>, {
    position?: Point;
    dispose?: (() => void) | null;
}> & EventHandlers;
export {};
