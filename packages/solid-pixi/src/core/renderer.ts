...
declare module 'pixi.js' {
  export interface Container {
    __SP_isContainer: true;
  }
}
Container.prototype.__SP_isContainer = true;
...
export type BaseInstance = Omit<
  Container,
  "children" | "attach" | "addChild" | "removeChild" | "raycast"
> & {
  __r3f: LocalState;
  children: Instance[];
...
function createThreeRenderer<TCanvas>(roots: Map<TCanvas, Root>, getEventPriority?: () => any) {
  ...
  function appendChild(parentInstance: Instance, child: Instance) {
    ...
    if (child) {
      ...
      } else if (child.__SP_isContainer && parentInstance.__SP_isContainer) {
  ...
  function insertBefore(parentInstance: Instance, child: Instance, beforeChild: Instance) {
    ...
    if (child) {
      if (child.__r3f.attach) {
        ...
        child.parent = parentInstance as unknown as Container;
...
export { prepare, createThreeRenderer as createPixiRenderer, extend };
