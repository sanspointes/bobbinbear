...
export const addEffect = (callback: GlobalRenderCallback) =>
  createSubs(callback, globalEffects);
export const addAfterEffect = (callback: GlobalRenderCallback) =>
  createSubs(callback, globalAfterEffects);
export const addTail = (callback: GlobalRenderCallback) =>
...
function render(timestamp: number, state: RootState) {
  // Run local effects
  let delta = state.ticker.deltaTime;
  ...
  // Render content
  if (!state.internal.priority && state.app.renderer.render)
