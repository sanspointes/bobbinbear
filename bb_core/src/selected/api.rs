...
use bevy_spts_changeset::{changes::ChangeSet, commands_ext::WorldChangesetExt, resource::ChangesetResource};
...
impl SelectedApi {
    fn query_selected_uids(world: &mut World) -> Vec<Uid> {
        let to_deselect: Vec<_> = world.query::<(&Uid, &Selected)>().iter(world).filter_map(|(uid, selected)| {
            match selected {
    ...
    pub fn set_object_selected(world: &mut World, uid: Uid, selected: Selected) -> Result<(), anyhow::Error> {
        let mut changeset = world.changeset();
    ...
    pub fn deselect_all_set_object_selected(world: &mut World, uid: Uid, selected: Selected) -> Result<(), anyhow::Error> {
        let to_deselect = SelectedApi::query_selected_uids(world);
...
impl SelectedApi {
    ...
    pub fn set_object_selected_js(world: &mut World, uid: String, selected: bool) -> Result<(), anyhow::Error> {
        SelectedApi::set_object_selected(world, Uid::try_from(&uid)?, selected.into())
    }

    pub fn deselect_all_set_object_selected_js(world: &mut World, uid: String, selected: bool) -> Result<(), anyhow::Error> {
